...
#' @examples
#' pal_ganyu()(10)
#' pal_ganyu(alpha = 0.5)(10)
#' pal_ganyu(alpha = 0.5, reverse = TRUE)(20)
pal_ganyu = function(alpha = 1, reverse = FALSE) {
	pal <- c("#33395B", "#5D74A2", "#C4D8F2", "#F2E8E3", "#7C282B")
	pal_rgb = col2rgb(pal)
	return(function(x){
		vec = 1:x
		if(reverse){
...
#' rgb_ganyu()
#' rgb_ganyu(10)
#' rgb_ganyu(n = 10, alpha = 0.5)
#' rgb_ganyu(n = 20, alpha = 0.7, reverse = TRUE)
rgb_ganyu = function(n = 12, alpha = 1, reverse = FALSE){
	return(pal_ganyu(alpha = alpha, reverse = reverse)(n))
}
...
#' @examples
#' scale_color_ganyu()
#' scale_color_ganyu(FALSE)
#' scale_color_ganyu(alpha = 0.5)
#' scale_color_ganyu(FALSE, alpha = 0.7, reverse = TRUE)
scale_color_ganyu = function(discrete = TRUE, alpha = 1, reverse = FALSE, ...) {
	if(discrete){
		return(discrete_scale("colour", "ganyu", pal_ganyu(alpha = alpha, reverse = reverse), ...))
	}
	else{
		return(scale_color_gradientn(colours = rgb_ganyu(n = 512, alpha = alpha, reverse = reverse), ...))
	}
}
...
#' @examples
#' scale_colour_ganyu()
#' scale_colour_ganyu(FALSE)
#' scale_colour_ganyu(alpha = 0.5)
#' scale_colour_ganyu(FALSE, alpha = 0.7, reverse = TRUE)
scale_colour_ganyu = scale_color_ganyu
...
#' @examples
#' scale_fill_ganyu()
#' scale_fill_ganyu(FALSE)
#' scale_fill_ganyu(alpha = 0.5)
#' scale_fill_ganyu(FALSE, alpha = 0.7, reverse = TRUE)
scale_fill_ganyu = function(discrete = TRUE, alpha = 1, reverse = FALSE, ...) {
	if(discrete){
		return(discrete_scale("fill", "ganyu", pal_ganyu(alpha = alpha, reverse = reverse), ...))
	}
	else{
		return(scale_fill_gradientn(colours = rgb_ganyu(n = 512, alpha = alpha, reverse = reverse), ...))
	}
}
