...
#' @examples
#' pal_kokomi()(10)
#' pal_kokomi(alpha = 0.5)(10)
#' pal_kokomi(alpha = 0.5, reverse = TRUE)(20)
pal_kokomi = function(alpha = 1, reverse = FALSE) {
	pal <- c("#2E2C69", "#46499C", "#8E8DC8", "#F7E2DB", "#D8A398")
	pal_rgb = col2rgb(pal)
	return(function(x){
		vec = 1:x
		if(reverse){
...
#' rgb_kokomi()
#' rgb_kokomi(10)
#' rgb_kokomi(n = 10, alpha = 0.5)
#' rgb_kokomi(n = 20, alpha = 0.7, reverse = TRUE)
rgb_kokomi = function(n = 12, alpha = 1, reverse = FALSE){
	return(pal_kokomi(alpha = alpha, reverse = reverse)(n))
}
...
#' @examples
#' scale_color_kokomi()
#' scale_color_kokomi(FALSE)
#' scale_color_kokomi(alpha = 0.5)
#' scale_color_kokomi(FALSE, alpha = 0.7, reverse = TRUE)
scale_color_kokomi = function(discrete = TRUE, alpha = 1, reverse = FALSE, ...) {
	if(discrete){
		return(discrete_scale("colour", "kokomi", pal_kokomi(alpha = alpha, reverse = reverse), ...))
	}
	else{
		return(scale_color_gradientn(colours = rgb_kokomi(n = 512, alpha = alpha, reverse = reverse), ...))
	}
}
...
#' @examples
#' scale_colour_kokomi()
#' scale_colour_kokomi(FALSE)
#' scale_colour_kokomi(alpha = 0.5)
#' scale_colour_kokomi(FALSE, alpha = 0.7, reverse = TRUE)
scale_colour_kokomi = scale_color_kokomi
...
#' @examples
#' scale_fill_kokomi()
#' scale_fill_kokomi(FALSE)
#' scale_fill_kokomi(alpha = 0.5)
#' scale_fill_kokomi(FALSE, alpha = 0.7, reverse = TRUE)
scale_fill_kokomi = function(discrete = TRUE, alpha = 1, reverse = FALSE, ...) {
	if(discrete){
		return(discrete_scale("fill", "kokomi", pal_kokomi(alpha = alpha, reverse = reverse), ...))
	}
	else{
		return(scale_fill_gradientn(colours = rgb_kokomi(n = 512, alpha = alpha, reverse = reverse), ...))
	}
}
