...
#' @examples
#' pal_xiao()(10)
#' pal_xiao(alpha = 0.5)(10)
#' pal_xiao(alpha = 0.5, reverse = TRUE)(20)
pal_xiao = function(alpha = 1, reverse = FALSE) {
	pal <- c("#333C42", "#316658", "#5EA69C", "#C2CFA2", "#A4799E", "#706690")
	pal_rgb = col2rgb(pal)
	return(function(x){
		vec = 1:x
		if(reverse){
...
#' rgb_xiao()
#' rgb_xiao(10)
#' rgb_xiao(n = 10, alpha = 0.5)
#' rgb_xiao(n = 20, alpha = 0.7, reverse = TRUE)
rgb_xiao = function(n = 12, alpha = 1, reverse = FALSE){
	return(pal_xiao(alpha = alpha, reverse = reverse)(n))
}
...
#' @examples
#' scale_color_xiao()
#' scale_color_xiao(FALSE)
#' scale_color_xiao(alpha = 0.5)
#' scale_color_xiao(FALSE, alpha = 0.7, reverse = TRUE)
scale_color_xiao = function(discrete = TRUE, alpha = 1, reverse = FALSE, ...) {
	if(discrete){
		return(discrete_scale("colour", "xiao", pal_xiao(alpha = alpha, reverse = reverse), ...))
	}
	else{
		return(scale_color_gradientn(colours = rgb_xiao(n = 512, alpha = alpha, reverse = reverse), ...))
	}
}
...
#' @examples
#' scale_colour_xiao()
#' scale_colour_xiao(FALSE)
#' scale_colour_xiao(alpha = 0.5)
#' scale_colour_xiao(FALSE, alpha = 0.7, reverse = TRUE)
scale_colour_xiao = scale_color_xiao
...
#' @examples
#' scale_fill_xiao()
#' scale_fill_xiao(FALSE)
#' scale_fill_xiao(alpha = 0.5)
#' scale_fill_xiao(FALSE, alpha = 0.7, reverse = TRUE)
scale_fill_xiao = function(discrete = TRUE, alpha = 1, reverse = FALSE, ...) {
	if(discrete){
		return(discrete_scale("fill", "xiao", pal_xiao(alpha = alpha, reverse = reverse), ...))
	}
	else{
		return(scale_fill_gradientn(colours = rgb_xiao(n = 512, alpha = alpha, reverse = reverse), ...))
	}
}
