...
#' @examples
#' pal_sayu()(10)
#' pal_sayu(alpha = 0.5)(10)
#' pal_sayu(alpha = 0.5, reverse = TRUE)(20)
pal_sayu = function(alpha = 1, reverse = FALSE) {
	pal <- c("#5B4A47", "#8C5D42", "#DA9464", "#F2D8AE", "#809A54")
	pal_rgb = col2rgb(pal)
	return(function(x){
		vec = 1:x
		if(reverse){
...
#' rgb_sayu()
#' rgb_sayu(10)
#' rgb_sayu(n = 10, alpha = 0.5)
#' rgb_sayu(n = 20, alpha = 0.7, reverse = TRUE)
rgb_sayu = function(n = 12, alpha = 1, reverse = FALSE){
	return(pal_sayu(alpha = alpha, reverse = reverse)(n))
}
...
#' @examples
#' scale_color_sayu()
#' scale_color_sayu(FALSE)
#' scale_color_sayu(alpha = 0.5)
#' scale_color_sayu(FALSE, alpha = 0.7, reverse = TRUE)
scale_color_sayu = function(discrete = TRUE, alpha = 1, reverse = FALSE, ...) {
	if(discrete){
		return(discrete_scale("colour", "sayu", pal_sayu(alpha = alpha, reverse = reverse), ...))
	}
	else{
		return(scale_color_gradientn(colours = rgb_sayu(n = 512, alpha = alpha, reverse = reverse), ...))
	}
}
...
#' @examples
#' scale_colour_sayu()
#' scale_colour_sayu(FALSE)
#' scale_colour_sayu(alpha = 0.5)
#' scale_colour_sayu(FALSE, alpha = 0.7, reverse = TRUE)
scale_colour_sayu = scale_color_sayu
...
#' @examples
#' scale_fill_sayu()
#' scale_fill_sayu(FALSE)
#' scale_fill_sayu(alpha = 0.5)
#' scale_fill_sayu(FALSE, alpha = 0.7, reverse = TRUE)
scale_fill_sayu = function(discrete = TRUE, alpha = 1, reverse = FALSE, ...) {
	if(discrete){
		return(discrete_scale("fill", "sayu", pal_sayu(alpha = alpha, reverse = reverse), ...))
	}
	else{
		return(scale_fill_gradientn(colours = rgb_sayu(n = 512, alpha = alpha, reverse = reverse), ...))
	}
}
